...
userController.register = catchAsync(async (req, res, next) => {
  let { name, email, password, position, phone1, teamId } = req.body;
  let user = await User.findOne({ email });
  if (user) throw new AppError(400, "User already", "Registration Error");
  const salt = await bcrypt.genSalt(10);
  password = await bcrypt.hash(password, salt);
  user = await User.create({ name, email, password, position, team: teamId, phone1 });
  let team = null;
  if (teamId) {
    user = await User.findById(user._id).populate("team");
    team = await Team.findByIdAndUpdate({ _id: teamId }, { $push: { workers: user._id } }, { new: true }).populate("manager");
...
userController.getCurrentUserAdmin = catchAsync(async (req, res, next) => {
  ...
  const status = await Task.find({
    $and: [{ doneAt: { $lte: date } }, { doneAt: { $gte: date1 } }],
  });
  ...
  const taskCount = {
    countTaskStatusArchive,
    countTaskStatusWorking,
    countTaskStatusReview,
    countTaskStatusDone,
    countTaskStatusRework,
    totalTask,
  };
  return sendResponse(
    res,
    200,
    true,
    { userCount, teamCount, projectCount, taskCount, countOutOfDate, countOnTime },
    null,
...
userController.updatePorfile = catchAsync(async (req, res, next) => {
  ...
  const allows = ["name", "avatarUrl", "email", "phone1", "phone2"];
...
userController.deleteingleUser = catchAsync(async (req, res, next) => {
  ...
  if (!!user.team) {
    if (user.position === "Manager") {
      team = await Team.findByIdAndUpdate({ _id: user.team }, { manager: null }, { new: true });
    }
    if (user.position === "Worker") {
      team = await Team.findByIdAndUpdate({ _id: user.team }, { $pull: { workers: user._id } }, { new: true });
    }
    await team.populate("workers");
